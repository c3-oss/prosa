  ...
  it('reads every projection and CAS count and calculates total rows', () => {
    const counts = readUploadCounts(
      fakeBundle([
        ...
        { sqlIncludes: 'FROM tool_calls', count: 13 },
        { sqlIncludes: 'FROM tool_results', count: 17 },
        { sqlIncludes: 'FROM messages', count: 23 },
        { sqlIncludes: 'FROM content_blocks', count: 29 },
        { sqlIncludes: 'FROM events', count: 31 },
        { sqlIncludes: 'FROM artifacts', count: 37 },
        { sqlIncludes: 'FROM objects', count: 11 },
        { sqlIncludes: 'size_bytes > ?', count: 19 },
      ]),
