...
description: Gate runner for Docker-backed Ralph Loop validation and reproducibility evidence.
...
Use this agent to run or inspect end-to-end validation gates for a Ralph Loop
feature.
...
- Read the matched domain skill before deciding which gates are required.
- If the feature touches server sync, read
  `.codex/skills/prosa-server-sync/SKILL.md` and
  `docs/architecture/server-sync.md`.
- Inspect available `just`, `pnpm`, and Docker commands before running anything.
