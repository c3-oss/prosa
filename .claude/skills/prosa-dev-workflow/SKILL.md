...
description: Local development workflow for the prosa TypeScript CLI. Use when setting up the repo, running CLI commands, choosing validation commands, understanding layout, or making general changes.
---
...
Prefer `devbox shell` before project commands when available.
...
For importer or schema work, also run affected focused tests, for example `pnpm test test/importers/codex.test.ts`.
