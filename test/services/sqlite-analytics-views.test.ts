...
describe('SQLite analytics views', () => {
  it('creates analytics views in the SQLite schema and counts match canonical tables', async () => {
    ...
    try {
      ...
      expect(queryCount(t.bundle.db, 'SELECT count(*) AS n FROM project_activity')).toBeGreaterThan(
        0,
      );
      expect(
        queryCount(t.bundle.db, `SELECT count(*) AS n FROM model_usage WHERE model = 'gpt-5.4'`),
      ).toBe(1);
