...
export class CodexProvider implements Provider {
  ...
  async parseAndProject(input: ProviderProjectInput): Promise<ProviderProjectResult> {
    ...
    // message that has at least one indexable text content block.
    // Shared with claude / cursor / gemini / hermes via
    // `buildSearchDocsFromMessageBlocks`; the v2 importers do not
    // yet port v1's per-tool-call / per-tool-result fan-out.
