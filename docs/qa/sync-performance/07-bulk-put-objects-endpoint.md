...
- **CAS dedup/race**: `putIfAbsent` é idempotente em casos comuns, mas S3 usa
  `IfNoneMatch: '*'` e FS/memory têm check-then-write. O bulk precisa tratar
  corrida same-key como sucesso apenas depois de `head` compatível.
