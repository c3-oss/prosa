...
| `just test-all` | yes | pass | **13/13** turbo at HEAD post-Tantivy index-dir probe landing. Focused counts: `@c3-oss/prosa-types-v2` 89, `@c3-oss/prosa-wire-v2` 21, conformance **26** (15 leaves + 6 providers-v2 projection-id + 5 bundle-compile idempotency), `@c3-oss/prosa-bundle-v2` **120**, `@c3-oss/prosa-importers-v2` **40**, `@c3-oss/prosa-derived-v2` **120** (Lane 3: scaffold policies + SessionBlobPackV2 byte layout + Parquet compaction planner + DuckDB analytics view shape contract + Tantivy schema/rebuild planner + projection-bridge with CQ-091 multibyte-preview coverage + cross-page transcript iterator with fragment coalescing + IndexCheckpointV2 canonical-JSON persistence with CQ-093 rename-based atomic replacement + `planAnalyticsExecution` composer + Tantivy index-dir best-effort probe), `@c3-oss/prosa-db-v2` 6. |
...
| 03 | `pnpm --filter @c3-oss/prosa-derived-v2 test` | yes | pass | **120 tests** / 13 files: SessionBlobPackV2 joint-constraint policy (11) + Parquet compaction trigger policy (6) + framing tests (8; CQ-084) + writer/reader round-trip (11; CQ-085) + Parquet compaction planner (8) + DuckDB analytics views shape contract incl. CQ-089 live+compacted overlay (11) + Tantivy schema/fingerprint (7) + Tantivy rebuild-plan state machine (10) + SessionBlobPackV2 projection-to-input bridge (9: 7 baseline + 2 CQ-091 UTF-8-byte regressions) + SessionBlobPackV2 cross-page transcript iterator (7) + IndexCheckpointV2 persistence (11) + `planAnalyticsExecution` SQL composer (9) + Tantivy index-dir probe (12: missing dir, file-not-dir, missing meta.json, malformed JSON, JSON-array root, missing `segments`, non-array `segments`, empty-segments happy path, populated-segments happy path, dangling symlink, path getters for `tantivyIndexDir` and `tantivyMetaPath`). Tantivy native binding + DuckDB runtime executor + runtime Parquet merge still pending. |
...
- [ ] No open blocking corrections. *(`CQ-091`, `CQ-092`, and `CQ-093` are
  all closed at HEAD; Lane 2 acceptance still pending Codex/governor/user
  sign-off.)*
...
- [ ] Security, integrity, remote-read, and E2E reviewer findings resolved
  ...
  atomic-replacement fix for the Tantivy checkpoint store.
