...
| `just test-all` | yes | pass | **13/13** turbo at HEAD post-`planTantivyRebuildFromBundle` orchestration helper landing. Focused counts: `@c3-oss/prosa-types-v2` 89, `@c3-oss/prosa-wire-v2` 21, conformance **26** (15 leaves + 6 providers-v2 projection-id + 5 bundle-compile idempotency), `@c3-oss/prosa-bundle-v2` **120**, `@c3-oss/prosa-importers-v2` **40**, `@c3-oss/prosa-derived-v2` **131** (Lane 3 surfaces: byte-layout, compaction, analytics views/composer, Tantivy schema/rebuild planner/index-dir probe/checkpoint persistence with atomic replacement/bundle-aware planner orchestration, SessionBlob projection-bridge + cross-page transcript iterator with fragment coalescing), `@c3-oss/prosa-db-v2` 6. |
...
| 03 | `pnpm --filter @c3-oss/prosa-derived-v2 test` | yes | pass | **131 tests** / 14 files: SessionBlobPackV2 joint-constraint policy (11) + Parquet compaction trigger policy (6) + framing tests (8; CQ-084) + writer/reader round-trip (11; CQ-085) + Parquet compaction planner (8) + DuckDB analytics views shape contract incl. CQ-089 live+compacted overlay (11) + Tantivy schema/fingerprint (7) + Tantivy rebuild-plan state machine (10) + SessionBlobPackV2 projection-to-input bridge (9) + SessionBlobPackV2 cross-page transcript iterator (7) + IndexCheckpointV2 persistence (11) + `planAnalyticsExecution` SQL composer (9) + Tantivy index-dir probe (14, incl. 2 CQ-094 escape-symlink regressions) + `planTantivyRebuildFromBundle` orchestration (9: fresh bundle, valid dir + no checkpoint, skip / incremental / fingerprint_mismatch / overwrite / index_dir_invalid / prior_run_failed / corrupt-checkpoint propagation). Tantivy native binding + DuckDB runtime executor + runtime Parquet merge still pending. |
