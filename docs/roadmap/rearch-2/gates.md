...
| 03 | `pnpm dev -- index-v2 status --help` | yes | pass | Implemented at `apps/cli/src/cli/commands/index-v2.ts`; prints "Print the combined Tantivy + SessionBlob status snapshot for a bundle v2 store." and `--store` requirement. Subprocess-spawned coverage: `apps/cli/test/cli/index-v2.test.ts` (17 tests across `status` + `sessions` + `epochs` + `transcript`: parent `--help` listing all four subcommands; `status` (`--help`, fresh-bundle empty snapshot, SessionBlob-populated snapshot, missing-`--store`); `sessions` (`--help`, fresh-bundle `[]`, multi-session multi-epoch inventory with `latest_epoch` correctness, missing-`--store`); `epochs` (`--help`, fresh-bundle `[]`, sorted deduplicated union of SessionBlob + projection epochs, missing-`--store`); `transcript` (`--help` documenting `--store` + `--session-id`, real zstd-compressed pack round-trip with epoch/pack_digest/messages assertions, unknown-session failure, missing-`--session-id` failure). |
