...
| `just test-all` | yes | pass | **13/13** turbo at HEAD post-SessionBlob listing + CQ-099 resolver-parity. Focused counts: `@c3-oss/prosa-types-v2` 89, `@c3-oss/prosa-wire-v2` 21, conformance **26** (15 leaves + 6 providers-v2 projection-id + 5 bundle-compile idempotency), `@c3-oss/prosa-bundle-v2` **120**, `@c3-oss/prosa-importers-v2` **40**, `@c3-oss/prosa-derived-v2` **214** (Lane 3 surfaces: byte-layout, compaction policy/planner/executor-plan, analytics views/composer, Tantivy schema/rebuild planner/index-dir probe with CQ-094 final-component + CQ-096 intermediate symlink rejection/checkpoint persistence with atomic replacement/bundle-aware planner orchestration/`clearTantivyIndexDir` reset helper, SessionBlob projection-bridge + cross-page transcript iterator with fragment coalescing, `derivedPaths` centralised layout + SessionBlob pack-path resolver + `loadSessionBlobPack` loader with CQ-094 final-component + CQ-098 intermediate symlink rejection), `@c3-oss/prosa-db-v2` 6. |
...
- [ ] No open blocking corrections. *(`CQ-099` is open for SessionBlob
  listing resolver-parity. Lane 2 is formally accepted by
  Codex/governor as of 2026-05-19.)*
...
- [ ] Security, integrity, remote-read, and E2E reviewer findings resolved
  ...
  SessionBlob pack loader; `CQ-099` is open for SessionBlob listing
  resolver-parity.
