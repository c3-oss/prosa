...
| `just test-all` | yes | pass | **13/13** turbo at HEAD post-`clearTantivyIndexDir` reset-helper. Focused counts: `@c3-oss/prosa-types-v2` 89, `@c3-oss/prosa-wire-v2` 21, conformance **26** (15 leaves + 6 providers-v2 projection-id + 5 bundle-compile idempotency), `@c3-oss/prosa-bundle-v2` **120**, `@c3-oss/prosa-importers-v2` **40**, `@c3-oss/prosa-derived-v2` **153** (Lane 3 surfaces: byte-layout, compaction policy/planner/executor-plan, analytics views/composer, Tantivy schema/rebuild planner/index-dir probe/checkpoint persistence with atomic replacement/bundle-aware planner orchestration/`clearTantivyIndexDir` reset helper, SessionBlob projection-bridge + cross-page transcript iterator with fragment coalescing, `derivedPaths` centralised layout), `@c3-oss/prosa-db-v2` 6. |
...
| 03 | `pnpm --filter @c3-oss/prosa-derived-v2 test` | yes | pass | **153 tests** / 17 files: SessionBlobPackV2 joint-constraint policy (11) + Parquet compaction trigger policy (6) + framing tests (8; CQ-084) + writer/reader round-trip (11; CQ-085) + Parquet compaction planner (8) + DuckDB analytics views shape contract incl. CQ-089 live+compacted overlay (11) + Tantivy schema/fingerprint (7) + Tantivy rebuild-plan state machine (10) + SessionBlobPackV2 projection-to-input bridge (9) + SessionBlobPackV2 cross-page transcript iterator (7) + IndexCheckpointV2 persistence (11) + `planAnalyticsExecution` SQL composer (9) + Tantivy index-dir probe (14) + `planTantivyRebuildFromBundle` orchestration (9) + `planCompactionExecution` composer (8) + `derivedPaths` centralised layout (7: pinned canonical paths, root passthrough, `derivedRoot` parity, no-drift delegation from `tantivyIndexDir`/`tantivyMetaPath`/`tantivyCheckpointPath`, relative-root composition) + `clearTantivyIndexDir` reset helper (7: fresh-bundle idempotency, repeated-invocation idempotency, stale-segment recursive removal, CQ-094 symlinked-index refusal preserving external target, regular-file-at-index refusal, symlinked-children unlink-in-place semantics, sibling-surface preservation). Tantivy native binding + DuckDB runtime executor + runtime Parquet merge still pending. |
...
- [x] No open blocking corrections. *(`CQ-091`..`CQ-095` are all closed;
