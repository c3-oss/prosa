...
| `just test-all` | yes | pass | 12/12 turbo at HEAD post-Hermes full per-record projection (Codex + Claude + Gemini + Hermes ship full projection on canonical schema fields). Focused counts: `@c3-oss/prosa-types-v2` 89, `@c3-oss/prosa-wire-v2` 21, conformance 15, `@c3-oss/prosa-bundle-v2` **120**, `@c3-oss/prosa-importers-v2` **39** (+4 CQ-074 full-projection assertions across Codex/Claude/Gemini/Hermes), `@c3-oss/prosa-db-v2` 6. |
...
| 02 | `pnpm --filter @c3-oss/prosa-importers-v2 typecheck` | yes | pass | Codex + Claude + Gemini + Hermes ship full per-record projection on canonical schema fields. |
| 02 | `pnpm --filter @c3-oss/prosa-importers-v2 test` | yes | pass | 39 tests / 7 files (GraphResolver 5, orchestrator 3, CodexProvider 7 incl. CQ-074 full-projection assertions, ClaudeProvider 7 incl. CQ-068 spawned-edge tests + CQ-074 Claude full-projection assertions, CursorProvider 4 incl. CQ-070 stable-key fix, GeminiProvider 6 incl. CQ-074 Gemini full-projection assertions, HermesProvider 7 incl. CQ-074 Hermes full-projection assertions). |
