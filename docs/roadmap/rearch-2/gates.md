...
| 03 | `pnpm --filter @c3-oss/prosa-derived-v2 test` | yes | pass | **44 tests** / 5 files: SessionBlobPackV2 joint-constraint policy (11) + Parquet compaction trigger policy (6) + framing round-trip / hash binding / boundary-condition tests (8; CQ-084) + writer/reader round-trip + atomic-fragment + pack_digest-recomputation + adversarial-single-message-over-1MiB + page-byte-cap tests (11; CQ-085) + Parquet compaction planner end-to-end on simulated bundle layouts (8: empty-plan / file-count trigger / low-count byte ceiling / multi-entity grouping / compact-NNNN re-run skip / sequence-number discovery / non-numeric entry skip). Tantivy writer, DuckDB analytics views, and the runtime Parquet merge still pending. |
