...
| 03 | `pnpm dev -- index-v2 status --help` | yes | pass | Implemented at `apps/cli/src/cli/commands/index-v2.ts`; prints "Print the combined Tantivy + SessionBlob status snapshot for a bundle v2 store." and `--store` requirement. Subprocess-spawned coverage: `apps/cli/test/cli/index-v2.test.ts` (9 tests: parent `--help` listing `status` + `sessions`, `status --help`, fresh-bundle empty snapshot, SessionBlob-populated snapshot, missing-`--store` failure for `status`, `sessions --help`, fresh-bundle `sessions` returning `[]`, multi-session multi-epoch `sessions` inventory with `latest_epoch` correctness, missing-`--store` failure for `sessions`). |
