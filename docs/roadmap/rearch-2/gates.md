...
| `just test-all` | yes | pass | 12/12 turbo at HEAD post-CQ-075/CQ-076 (Codex full per-record projection on canonical schema fields). Focused counts: `@c3-oss/prosa-types-v2` 89, `@c3-oss/prosa-wire-v2` 21, conformance 15, `@c3-oss/prosa-bundle-v2` **120**, `@c3-oss/prosa-importers-v2` **36** (+1 CQ-074 Codex full-projection assertion on canonical fields), `@c3-oss/prosa-db-v2` 6. |
...
| 02 | `pnpm --filter @c3-oss/prosa-importers-v2 typecheck` | yes | pass | CQ-075 + CQ-076 closed: Codex full per-record projection ships with no `as never` casts and only canonical schema fields on ToolCallV2/ToolResultV2/ContentBlockV2. |
| 02 | `pnpm --filter @c3-oss/prosa-importers-v2 test` | yes | pass | 36 tests / 7 files (GraphResolver 5, orchestrator 3, CodexProvider 7 incl. CQ-074 full-projection canonical-field assertions, ClaudeProvider 6 incl. CQ-068 spawned-edge tests, CursorProvider 4 incl. CQ-070 stable-key fix, GeminiProvider 5, HermesProvider 6). |
