...
| `just test-all` | yes | pass | **13/13** turbo at HEAD post-`listCompactedOutputs` + `prosa index-v2 compacted-outputs` manifest-vs-on-disk consistency audit landing. Focused counts: `@c3-oss/prosa-types-v2` 89, `@c3-oss/prosa-wire-v2` 21, conformance **26** (15 leaves + 6 providers-v2 projection-id + 5 bundle-compile idempotency), `@c3-oss/prosa-bundle-v2` **120**, `@c3-oss/prosa-importers-v2` **40**, `@c3-oss/prosa-derived-v2` **470** (Lane 3 surfaces: byte-layout, compaction policy/planner/executor-plan, analytics views/composer, Tantivy schema/rebuild planner/index-dir probe with CQ-094 final-component + CQ-096 intermediate symlink rejection/checkpoint persistence with atomic replacement/bundle-aware planner orchestration/`clearTantivyIndexDir` reset helper, SessionBlob projection-bridge + cross-page transcript iterator with fragment coalescing, `derivedPaths` centralised layout + SessionBlob pack-path resolver + `loadSessionBlobPack` loader with CQ-094 final-component + CQ-098 intermediate symlink rejection), `@c3-oss/prosa-db-v2` 6. |
