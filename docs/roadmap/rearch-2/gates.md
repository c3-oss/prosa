...
| 03 | `pnpm dev -- index-v2 status --help` | yes | pass | Implemented at `apps/cli/src/cli/commands/index-v2.ts`; prints "Print the combined Tantivy + SessionBlob status snapshot for a bundle v2 store." and `--store` requirement. Subprocess-spawned coverage: `apps/cli/test/cli/index-v2.test.ts` (5 tests: parent `--help`, `status --help`, fresh-bundle empty snapshot, SessionBlob-populated snapshot, missing-`--store` failure). |
