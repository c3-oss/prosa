...
Current HEAD: pending (`summariseDerivedLayerFootprint` per-subsystem byte/file rollup + `prosa index-v2 footprint` CLI subcommand).
No-change streak: 0
Ralph active: yes
...
- `CQ-112`: `summariseDerivedLayerFootprint()` must account for or
  refuse every direct child of `<bundleRoot>/derived/`; unknown
  top-level regular files count in `other`, and unknown top-level
