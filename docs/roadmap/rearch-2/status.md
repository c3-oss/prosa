...
Current HEAD: pending (`derivedLayerSnapshot` MCP-friendly bulk read + `prosa index-v2 snapshot` CLI subcommand).
No-change streak: 0
Ralph active: yes
...
- `CQ-113`: snapshot WIP must not plant malformed Tantivy checkpoint
  JSON in a test that expects `derivedLayerSnapshot()` to succeed.
