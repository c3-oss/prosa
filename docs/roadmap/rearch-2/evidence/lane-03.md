...
- [x] Compact manifest persistence (`writeCompactManifestV2` +
  `readCompactManifestV2` + `compactManifestPath`) in
  `@c3-oss/prosa-derived-v2/src/compaction/manifest.ts`. Writes
  the manifest to `<bundleRoot>/epochs/compact-<NNNN>/compact.manifest.json`
  via the CQ-093 atomic-rename + parent-fsync pattern; reader
  validates schema discriminator + full per-entity / per-superseded
  shape (CQ-107: every entity must have non-empty `entity_type`,
  known `reason`, non-empty `output_path`, non-negative integer
  ...
  manifest path is a symlink (parallel to CQ-094/CQ-098/CQ-103
  containment elsewhere in derived-v2). The Lane 3 spec calls
  the file `.cbor`; we keep `.json` until a canonical-CBOR
...
- [x] `prosa index-v2` CLI group (`apps/cli/src/cli/commands/index-v2.ts`)
  ...
  `listCompactedOutputs` + `planSupersededCleanup` into one
  JSON snapshot (status + projection + compaction-plan +
  persisted-compactions consistency + GC partition);
  `next-action` is the prescriptive layer over the same
  summary — wires `recommendMaintenanceActions(summary)` to
  return the ordered list of recommended next actions
  (`resume_compaction` first when an inconsistent persisted
  compaction would block GC, then `gc_superseded` when safe
  candidates exist, then `run_compaction` when the planner
  would fire; `[]` when the bundle is idle); `status`
  ...
  --session-id })` when `--session-id` is supplied (result
  wrapped in an array — `[summary]` if the session has packs,
  `[]` if not — so consumers always see the same JSON shape); `epochs` wires
  ...
  `tantivy-schema` emits
  `{ fingerprint: currentTantivySchemaFingerprint(), fields:
  TANTIVY_SCHEMA_FIELDS }` (content-free, takes no `--store`;
  Tantivy-side counterpart to `analytics-views`);
  ...
  `planCompaction(--store)`; `compaction-manifest` is the
  unified persisted-manifest surface: default mode generates
  from the current plan via `planCompaction(--store)` +
  `buildCompactManifestV2({ plan, generatedAt:
  --generated-at })` and prints; `--write` atomically persists
  via `writeCompactManifestV2(--store, manifest)` and prints
  `{ manifest, persisted_path }`; `--read --compaction-seq <n>`
  reads a persisted manifest via `readCompactManifestV2(--store,
  n)` (CQ-107 deep-validated); `--read` + `--write` mutually
  exclusive and rejected up-front; `compaction-execution-plan` composes
  `planCompaction(--store)` then `planCompactionExecution({
  bundleRoot, plan })`; `superseded-segments` wires
  `listSupersededSegmentsFromManifests(--store)` (flat) and
  ...
  `compact.manifest.json` files; `gc-plan` wires
  `planSupersededCleanup(--store)`: composes
  `superseded-segments` + `compacted-outputs` to produce one
  row per superseded segment tagged `safe_to_delete: true|false`
  (true iff the declaring compaction-seq is in a consistent
  post-merge state); pure-read, the planner never deletes;
  `derived-layout` wires `derivedPaths(--store)`: emits the
  resolved absolute paths for the derived-layer subsystems
  (`root`, `derived`, `tantivy`, `tantivyIndex`, `tantivyMeta`,
  `tantivyCheckpoint`, `sessionBlob`, `analytics`). Pure path
  resolution — does not touch the filesystem, so it works on a
  non-existent store and is safe to call from ops scripts that
  need to discover where to look without hardcoding the
  layout.
  `snapshot` wires `derivedLayerSnapshot(--store)`: one-call bulk
  read combining `maintenance` + `recommendations` + `footprint`
  + `capabilities` into a single coherent JSON object. The
  recommendations layer reads from the same maintenance summary
  surfaced in the snapshot (no race / no double-read), so an MCP
  server or dashboard can branch on the recommendations and
  cross-reference back to the maintenance fields they came from.
  Pure-read. Spares downstream tools the four round-trips.
  `capabilities` wires `derivedLayerCapabilities()`: content-free
  introspection (takes no `--store`) emitting the schema
  discriminators (`compact_manifest:
  'prosa.compact-manifest.v2'`), compaction fire-reasons +
  policy thresholds (`small_segment_bytes`, `file_count_trigger`,
  `low_count_trigger`, `low_count_byte_ceiling`), analytics
  entity tables + view names, and the Tantivy schema fingerprint
  + field list. One stable shape for downstream tools / MCP
  servers / parsers to discover what the binary supports.
  `footprint` wires `summariseDerivedLayerFootprint(--store)`:
  walks `<bundleRoot>/derived/` and reports `{ total_bytes,
  session_blob, tantivy, analytics, other }` where each subsystem
  row carries `{ byte_count, file_count, present }`. Refuses to
  follow symlinks at the subsystem root, intermediate
  directories, or the `derived/` root itself (CQ-094 parallel
  containment). Forward-compatible: unknown subdirectories under
  ...
  superseded_segment_count }` rows sorted by `compaction_seq`
  ascending (also chronological in practice). `generated_at` is
  echoed verbatim from the persisted manifest, not recomputed.
  Pure-read; the audit view that answers "when did each
  compaction run?".
  `compaction-effectiveness` wires
  `summariseCompactionEffectiveness(--store)`: per-compaction-seq
  bytes-in (`sum(superseded[].byte_length)`) vs bytes-out
  (`sum(entity_outputs[].byte_length)`), `bytes_saved`, and
  `reduction_ratio` plus `{ consistent_count, inconsistent_count,
  bytes_in_consistent, bytes_out, bytes_saved, reduction_ratio }`
  totals across the consistent subset. Inconsistent rows surface
  in the listing with `bytes_out: null` / `bytes_saved: null` /
  `reduction_ratio: null` and are excluded from top-level totals.
  Pure-read; the post-hoc "was it worth it?" counterpart to
  `compaction-execution-plan` ("how do we run it?").
  `gc-execution-plan` wires `planGcExecution(--store)`: the GC-side
  execution-plan counterpart that drops blocked rows and emits one
  deterministic unlink-step per safe candidate (`{ path, byte_length,
  epoch, entity_type, compaction_seq }`) with a top-level
  `{ empty, total_bytes, steps }` shape — pure-read, the audit /
  dry-run surface the eventual runtime GC executor will bind to;
  `compacted-outputs` wires
  ...
  non-zero when any failure is captured); `transcript-header`
  wires `readSessionBlobHeader({ bundleRoot:
  ...
  sessionId: --session-id })`. All eighteen print pretty JSON to
  ...
  `transcript` accepts `--format json` (default), `--format text`,
  or `--format markdown`, with optional `--start-ordinal <n>` /
  `--end-ordinal <n>` inclusive bounds that propagate to
  `loadTranscriptFromBundle`'s `range` so out-of-range pages are
  skipped without decompression, and an optional `--epoch <n>`
  that selects a specific historical pack instead of the latest.
  `loadTranscriptFromBundle` now accepts an optional `epoch` of
  its own, branching internally between `loadLatestSessionBlobPack`
  (when omitted) and `loadSessionBlobPack` (when supplied).
  `iterateTranscriptFromBundle` mirrors the same `epoch` parameter
  for streaming/paged consumers, with matching tests covering the
  historical-pack happy path and missing-epoch ENOENT. `text` uses `formatTranscriptTextV2`
  (flat ASCII rendering with `[#ordinal] role @ ts (turn: id)`
  sections). `markdown` uses `formatTranscriptMarkdownV2`
  (`# Transcript` heading + bulleted metadata + per-message
  `## #ordinal — role` sections; CQ-106 fence-escalation so
  inline payloads and cas_ref previews containing triple
  backticks still nest faithfully). CQ-105: the format flag is
  validated synchronously before any bundle read so an invalid
  format surfaces `invalid --format` rather than a misleading
  ...
  segments across epochs 4..20), then asserts that `status` /
  `sessions` / `epochs` / `projection-segments` (flat +
  `--summary`) / `compaction-plan` / `transcript` (latest +
  historical epoch) all produce mutually consistent JSON
  (session inventory matches between status and sessions,
  `epochs` equals the union of session-bearing + projection-
  ...
  consistent_count: 0, inconsistent_count: 1 }` with GC
  partition entirely blocked + planted compacted output flips
  to `{ consistent_count: 1 }` with GC fully safe_to_delete +
  missing-`--store` failure (5); `next-action --help` + idle
  bundle `[]` + fired planner only → `[run_compaction]` +
  persisted-manifest-without-output → `[resume_compaction,
  run_compaction]` (correctness-first priority) + planted
  compacted output → includes `gc_superseded` with
  `safe_count: 17` + missing-`--store` failure (6); `status`
  ...
  correctness + missing-`--store` + `--session-id <existing>`
  filters to `[summary]` with the correct `epochs[]` /
  `latest_epoch` + `--session-id <missing>` returns `[]`
  (6); `epochs --help` +
  ...
  `tantivy-schema --help` documenting the schema endpoint +
  no `--store <path>` option + canonical fields list (only
  `text` is `default`-tokenized; raw-tokenized include
  `doc_id`, `session_id`, `timestamp`; field order is
  fingerprint-locked starting with `doc_id`) + fingerprint
  matches the `status.tantivy.current_schema_fingerprint`
  emitted by the bundle-derived-status command (3);
  ...
  `compaction-manifest --help` documenting `--store` +
  `--generated-at` + 17-small-segments fired plan returns a
  manifest with `schema: 'prosa.compact-manifest.v2'`,
  `compaction_seq: 1`, the supplied `generated_at`, one entity
  (`sessions` / `low_count_byte_ceiling`) with 17 superseded
  segments and `total_bytes_in: 17 * 1024` + empty-plan
  refusal + `--write` mode persists manifest to disk and
  returns `{ manifest, persisted_path }` with the file
  actually present + `--read --compaction-seq 1` round-trips
  ...
  `gc-plan --help` documenting `--store` + fresh-bundle empty
  plan + post-`--write` without compacted output → all
  candidates `safe_to_delete: false, blocked_reason:
  'output_missing'` with the rollup totaling 17×1024 in
  `blocked` + planting the compacted output flips every
  candidate to `safe_to_delete: true, blocked_reason: null`
  with totals moving to `safe_to_delete` + missing-`--store`
  failure (5);
  `gc-execution-plan --help` documenting `--store` and the
  unlink-step shape + fresh-bundle `{ empty: true, total_bytes:
  0, steps: [] }` + manifest-without-output returns the same
  empty shape (all candidates blocked) + planted compacted
  output yields 17 steps with `entity_type: 'sessions'`,
  `compaction_seq: 1`, `byte_length: 1024` per step, and
  `total_bytes: 17 * 1024` + missing-`--store` failure (5);
  `derived-layout --help` documenting `--store` and the
  path-resolution contract + every canonical subsystem path
  resolves anchored at `--store` (`root` → `<store>`,
  `derived` → `<store>/derived`, `tantivyMeta` →
  `<store>/derived/tantivy/index/meta.json`, etc.) + does not
  touch the filesystem (succeeds on a non-existent store) +
  missing-`--store` failure (4);
  `snapshot --help` documenting `--store` and the bulk-read
  contract + fresh-bundle shape includes `maintenance.compaction.
  empty: true`, `recommendations: []`, `footprint.total_bytes:
  0`, and the canonical `capabilities.schema_ids.compact_manifest`
  + planted 17 small projection segments → snapshot
  recommendations are `[run_compaction]` and the surfaced
  `maintenance.compaction.reasons` is
  `['low_count_byte_ceiling']` (internal coherence) +
  missing-`--store` failure (4);
  `capabilities --help` documenting the content-free
  introspection contract (no `--store <path>` advertised) +
  capabilities emits schema discriminators, the compaction
  policy enum + thresholds, analytics view names + entity
  tables, and the Tantivy schema fingerprint + fields +
  determinism across invocations (same JSON output) (3);
  `footprint --help` documenting `--store` and the per-subsystem
  breakdown + fresh bundle all-zero shape with every subsystem
  `present: false` + planted session-blob packs (1024 + 2048
  across two pack files in `epoch-1`) + planted tantivy index
  files (100-byte checkpoint + 4096-byte segment under
  `tantivy/index/`) report `session_blob: { byte_count: 3072,
  ...
  compacted output emits one row with verbatim
  `generated_at: '2026-05-19T12:00:00.000Z'`, `consistent:
  false`, `entity_count: 1`, `superseded_segment_count: 17` +
  planting the compacted output flips `consistent: true` (same
  row, generated_at preserved) + missing-`--store` failure (4);
  `compaction-effectiveness --help` documenting `--store` and
  the rollup contract + fresh-bundle zero-state summary +
  manifest-without-output reports `consistent: false`,
  `bytes_in: 17 * 1024`, `bytes_out: null`,
  `missing_output_count: 1` with totals
  `{ consistent_count: 0, inconsistent_count: 1,
  bytes_in_consistent: 0 }` + planted compacted output
  reports `consistent: true`, `bytes_in: 17 * 1024`,
  `bytes_out: 2048`, `bytes_saved: 17 * 1024 - 2048`, and
  `reduction_ratio` matching `(17 * 1024 - 2048) / (17 * 1024)`
  + missing-`--store` failure (5);
  ...
  row `entity_type=sessions, compaction_seq=1,
  byte_length=1024`, epochs ascending 1..17) +
  `--summary` rollup totals (17 segments, 17*1024 bytes,
  by_entity.sessions, by_compaction_seq['1']) +
  missing-`--store` failure (5);
  `verify-packs --help` documenting `--store` + clean bundle
  with 2 packs verifies and exits 0 + corrupted pack lands in
  `failed[]` and exits 1 + fresh bundle returns empty arrays
  with exit 0 (4);
  ...
  `--format markdown` rendered with `# Transcript` heading +
  `- **epoch**: 3` metadata + `- **message_count**: 1` row +
  `## #0 — user` section + timestamp/turn line +
  `--format json` explicit + CQ-105 `--format yaml` rejected
  BEFORE any bundle read (stderr matches `invalid --format`,
  does not mention `loadLatestSessionBlobPack`, and the error
  enumerates all three supported formats) + `--start-ordinal` +
  `--end-ordinal` filter messages to the inclusive ordinal range
  (4 planted messages with `--start-ordinal 1 --end-ordinal 2`
  returns ordinals [1, 2]) + inverted range rejected with
  `invalid range: --start-ordinal (5) > --end-ordinal (2)` +
  negative `--start-ordinal` rejected synchronously +
  `--epoch <n>` reads a specific historical pack (3 epochs
  planted at 1/4/9 with 2/3/7 messages → default returns epoch 9
  with 7 messages, `--epoch 4` returns epoch 4 with 3 messages
  and `path` containing `epoch-4`) + `--epoch 99` against a
  bundle without that epoch surfaces ENOENT verbatim (13).
  Apps/cli typecheck +
