...
- [x] `prosa index-v2` CLI group (`apps/cli/src/cli/commands/index-v2.ts`)
  shipping twenty-one pure-read subcommands. `maintenance` wires
  ...
  108 subprocess tests (`apps/cli/test/cli/index-v2.test.ts`) cover:
  parent `--help` listing all twenty-one subcommands,
...
pnpm --filter @c3-oss/prosa exec vitest run test/cli/index-v2.test.ts  # 108 subprocess-spawned tests
