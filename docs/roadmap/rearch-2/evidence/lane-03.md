...
- [x] `prosa index-v2` CLI group (`apps/cli/src/cli/commands/index-v2.ts`)
  shipping four pure-read subcommands. `status` wires
  `bundleDerivedStatus(--store)`; `sessions` wires
  `listSessionBlobSummaries(--store)`; `epochs` wires
  `derivedLayerEpochsTouched(--store)`; `transcript` wires
  `loadTranscriptFromBundle({ bundleRoot: --store, sessionId:
  --session-id })`. All four print pretty JSON to stdout — no
  native bindings, no filesystem mutation. The `index-v2` parent
  command is the eventual home for `tantivy` (blocked on the
  native binding); `status`, `sessions`, `epochs`, and
  `transcript` ship now and stand in for the JSON-form path of
  gate criterion 3 (`prosa session show <id>` against a v2
  bundle — v1-renderer parity output is a follow-up).
  17 subprocess tests (`apps/cli/test/cli/index-v2.test.ts`) cover:
  parent `--help` listing all four subcommands, `status`
