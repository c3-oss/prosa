...
Source: `~/.claude/projects/<project-slug>/<session-id>.jsonl`, plus
subagent transcripts under `<session-id>/subagents/` (directly for
Agent-tool spawns, nested at `subagents/workflows/wf_<id>/` for
Workflow-tool spawns). Each line is a JSON object discriminated by
`type`. The full envelope reference is `docs/sources/claude-code.md`.
...
| `ParentSessionID` | subagents: the parent session UUID — the directory above the innermost `subagents` path component. nil for main sessions and when that directory is not UUID-shaped |
