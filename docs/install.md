...
| --- | --- | --- |
| Homebrew (macOS) | `brew install c3-oss/prosa/prosa` | Auto-updates with `brew upgrade`. Installs all three binaries. |
...
publishes all three binaries (`prosa`, `prosa-server`, `prosa-panel`) into
the Homebrew prefix. GoReleaser updates the cask on every tag push — there
is no manual step.
