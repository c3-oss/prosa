...
export async function compileHermes(
  ...
  try {
    ...
    if (sources.stateDbPath) {
      ...
      const dbCandidates = readSqliteCandidates(sources.stateDbPath, sourceFile)
    ...
    const selected = selectCandidates(candidates).filter((candidate) => candidate.sourceFile.imported)
...
function selectCandidates(candidates: SessionCandidate[]): SessionCandidate[] {
...
async function buildPending(
  ...
  const selectedSourceIds = new Set(selected.map((candidate) => candidate.sourceFile.sourceFileId))
  for (const sourceFile of importedSources) {
    if (selectedSourceIds.has(sourceFile.sourceFileId)) continue
  ...
  for (const candidate of selected) {
    await stageCandidate(batch, candidate, pending)
  }
  buildSearchDocs(pending)
  return pending
}

async function stageCandidate(batch: ImportBatch, candidate: SessionCandidate, pending: PendingState): Promise<void> {
  ...
  if (candidate.session.systemPrompt) {
    ...
    await pushTextBlock(pending, sessionPk, messageId, null, 0, 'text', candidate.session.systemPrompt, rawRecordId)
  }

  const toolCallIds = new Map<string, string>()
  for (const message of candidate.messages) {
    await stageMessage(candidate, sessionPk, message, batch.batch_id, pending, toolCallIds)
  }
}
...
async function stageMessage(
  ...
  await pushHiddenBlock(pending, sessionPk, messageId, eventId, message, rawRecordId)
  ...
  for (const [index, call] of parseToolCalls(message.toolCalls).entries()) {
    ...
    toolCallIds.set(sourceCallId, toolCallId)
  ...
  if (role === 'tool' && message.toolCallId) {
    ...
    let toolCallId = toolCallIds.get(sourceCallId)
    if (!toolCallId) {
      toolCallId = makeToolCallId(sessionPk, sourceCallId)
      toolCallIds.set(sourceCallId, toolCallId)
      pending.toolCalls.push({
        tool_call_id: toolCallId,
        session_id: sessionPk,
...
async function pushHiddenBlock(
  pending: PendingState,
  sessionId: string,
  messageId: string,
  eventId: string,
  ...
  for (const [kind, value] of hidden) {
    ...
    if (!text) continue
    await pushTextBlock(pending, sessionId, messageId, eventId, offset++, kind, text, rawRecordId, 'hidden_by_default')
  }
}
...
function buildSearchDocs(pending: PendingState): void {
  for (const message of pending.messages) {
    ...
    pending.searchDocs.push({
      ...
      project_id: pending.sessions.find((session) => session.session_id === message.session_id)?.project_id ?? null,
      timestamp: message.timestamp,
      role: message.role,
      tool_name: null,
  ...
  for (const call of pending.toolCalls) {
    ...
    pending.searchDocs.push({
      ...
      project_id: pending.sessions.find((session) => session.session_id === call.session_id)?.project_id ?? null,
  ...
  for (const result of pending.toolResults) {
    ...
    pending.searchDocs.push({
      ...
      project_id: pending.sessions.find((session) => session.session_id === result.session_id)?.project_id ?? null,
      timestamp: null,
      role: 'tool',
      tool_name: null,
      canonical_tool_type: null,
...
function flushPending(bundle: Bundle, pending: PendingState): void {
  ...
  const insertSession = prepare(
    ...
    `INSERT OR REPLACE INTO sessions (
     ...
     ) VALUES (?, 'hermes', ?, ?, ?, 0, NULL, NULL, ?, NULL, ?, ?, NULL, NULL, ?, ?, ?, 'high', ?)`,
  )
  for (const session of pending.sessions) {
    insertSession.run(
      session.session_id,
