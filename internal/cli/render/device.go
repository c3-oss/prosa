...
// map (id → friendly_name), the mapped value wins — trimmed to its
// first dot segment so FQDN-style hostnames ("ip-10-0-0-1.ec2.internal")
// read like `hostname -s`. Otherwise we fall back to the first 7 hex
// chars + "…", which at least signals the shape of the id while
// staying narrow enough for the row layout.
//
