...
func (h *AnalyticsHandler) GetReport(ctx context.Context, req *connect.Request[prosav1.GetReportRequest]) (*connect.Response[prosav1.GetReportResponse], error) {
	...
	default:
		return nil, connect.NewError(connect.CodeInvalidArgument,
			fmt.Errorf("unknown report %q (want sessions|tools|models|projects|errors|heatmap|usage|hours|usage_by_model|errors_by_model|usage_by_day|punchcard|durations|duration_stats|subagents)", req.Msg.Report))
	}
}
