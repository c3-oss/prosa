...
- **Not a place that mutates your data.** Raw `.jsonl` from each agent is
  ...
  directories (Grok Build — a session spans summary, chat, and a usage
  ledger, and no single file captures it). Projections are complete: every
  source line that informs the canonical session, including usage/cost
  provenance, lands in the projected artifact verbatim. Every other agent
  (one source file per session) is preserved byte-for-byte.
